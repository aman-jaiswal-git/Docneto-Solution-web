...
{	
    ...
    if(isset($_POST['availableDays'])){
        $availableDays = implode(',', $_POST['availableDays']); // Mon,Tue,Wed...
    }

    // ---------- Image Upload ----------
    $imageName = preg_replace('/\s+/', '_', strtolower($docname)); // spaces -> underscore
    $targetDir = "doctor-images/";   // ✅ आपके folder नाम से match किया गया
    if(!is_dir($targetDir)){
        mkdir($targetDir,0777,true);
    }
    $targetFile = $targetDir . $imageName . ".png";

    if(isset($_FILES["docphoto"]["tmp_name"]) && $_FILES["docphoto"]["tmp_name"]!=""){
        if(move_uploaded_file($_FILES["docphoto"]["tmp_name"], $targetFile)){
            $docImage = $targetFile;
        } else {
            $docImage = "";
        }
    ...
    // ---------- Insert Query ----------
    $sql = mysqli_query($con,"INSERT INTO doctors
	(specilization,doctorName,address,docFees,contactno,docEmail,password,docImage,available_days) 
	VALUES 
	('$docspecialization','$docname','$docaddress','$docfees','$doccontactno','$docemail','$password','$docImage','$availableDays')");

    if($sql){
        echo "<script>alert('Doctor info added Successfully');</script>";
        echo "<script>window.location.href ='manage-doctors.php'</script>";
    } else {
        echo "<script>alert('❌ Error: Doctor info not added');</script>";
    }
}
?>
...
<!-- Doctor Photo -->
<div class="form-group">
	<label for="docphoto">Doctor Photo</label>
	<input type="file" name="docphoto" class="form-control" accept="image/*" required="true">
</div>
...
<!-- ✅ Available Days -->
<div class="form-group">
	<label for="availableDays">Available Days</label><br>
